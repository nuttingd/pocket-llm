...
static std::mutex g_engine_mutex;  // guards load/unload/infer exclusion

// ---- Signal handler guard ----
//
// Native crashes (SIGSEGV/SIGBUS) in llama.cpp or the Vulkan driver would
// normally kill the entire process.  We install a signal handler that, when
// a crash occurs inside a guarded JNI call, jumps back to a safe point so
// the JNI function can throw a Java exception instead of dying.
//
// After a caught signal the native state (model, context, Vulkan resources)
// is assumed corrupted.  We null-out the global pointers (accepting the
// memory leak) and mark the engine as poisoned.  The Kotlin side must call
// nativeUnload() + reload before retrying.
...
static void crash_handler(int sig, siginfo_t *info, void *ucontext) {
    ...
    // Not our crash — chain to the previous handler (e.g. ART's).
    ...
    if (old->sa_flags & SA_SIGINFO) {
        ...
        // Re-raise with the default handler so the OS crash reporter fires.
...
static void install_crash_handlers() {
    if (g_handlers_installed.exchange(true)) return;   // already done
...
// Poison the native state after a caught crash.  We cannot safely call
// the normal free functions because the heap / Vulkan state may be corrupt,
// so we just null out the pointers and accept the leak.
...
Java_dev_nutting_pocketllm_llm_LlmEngine_nativeInit(JNIEnv *env, jobject, jobjectArray backendPaths) {
    ...
    // Log all registered devices after init
...
Java_dev_nutting_pocketllm_llm_LlmEngine_nativeLoadModel(
    ...
    int n_gpu_layers = -1; // fallback: offload all
    {
...
extern "C"
JNIEXPORT jstring JNICALL
Java_dev_nutting_pocketllm_llm_LlmEngine_nativeInfer(
    JNIEnv *env, jobject thiz,
    jbyteArray jImageBytes, jstring jPrompt, jint maxTokens
) {
    if (g_poisoned.load() || !g_model || !g_context || !g_mtmd) {
    ...
    if (caught_signal != 0) {
        ...
        LOGe("Caught %s during nativeInfer — poisoning native state", sig_name);
        poison_native_state();
    ...
    // Get image bytes
    report_progress(env, thiz, progressMid, "image_processing", 0);
    jsize img_len = env->GetArrayLength(jImageBytes);
    auto *img_data = (unsigned char *)env->GetByteArrayElements(jImageBytes, nullptr);

    // Create bitmap from image buffer
    mtmd_bitmap *bitmap = mtmd_helper_bitmap_init_from_buf(g_mtmd, img_data, img_len);
    env->ReleaseByteArrayElements(jImageBytes, (jbyte *)img_data, JNI_ABORT);

    if (!bitmap) {
        LOGe("Failed to create bitmap from image bytes");
    ...
    if (tmpl) {
        ...
        int32_t tmpl_len = llama_chat_apply_template(tmpl, chat, 1, true, nullptr, 0);
        if (tmpl_len > 0) {
            full_prompt.resize(tmpl_len);
            llama_chat_apply_template(tmpl, chat, 1, true, full_prompt.data(), full_prompt.size() + 1);
        ...
        full_prompt = user_content;
        LOGw("No chat template in model, using raw prompt");
    }
    ...
    int32_t eval_result = mtmd_helper_eval_chunks(
        g_mtmd, g_context, chunks, n_past, 0, BATCH_SIZE, true, &n_past
    );
    mtmd_input_chunks_free(chunks);
    ...
    // Token generation loop — free-form chat (no grammar constraints)
    report_progress(env, thiz, progressMid, "generating", 0);
    common_params_sampling sparams;

    // Read model-recommended sampling defaults from GGUF metadata
    auto meta_float = [&](enum llama_model_meta_key key, float &dst) {
        char buf[64] = {0};
        if (llama_model_meta_val_str(g_model, llama_model_meta_key_str(key), buf, sizeof(buf)) > 0) {
            char *end = nullptr;
            float v = strtof(buf, &end);
            if (end && end != buf) dst = v;
        }
    };
    auto meta_int = [&](enum llama_model_meta_key key, int32_t &dst) {
        char buf[64] = {0};
        if (llama_model_meta_val_str(g_model, llama_model_meta_key_str(key), buf, sizeof(buf)) > 0) {
            char *end = nullptr;
    ...
    meta_int  (LLAMA_MODEL_META_KEY_SAMPLING_TOP_K,          sparams.top_k);
    meta_float(LLAMA_MODEL_META_KEY_SAMPLING_TOP_P,          sparams.top_p);
    meta_float(LLAMA_MODEL_META_KEY_SAMPLING_MIN_P,          sparams.min_p);
    meta_float(LLAMA_MODEL_META_KEY_SAMPLING_TEMP,           sparams.temp);
    meta_float(LLAMA_MODEL_META_KEY_SAMPLING_PENALTY_REPEAT, sparams.penalty_repeat);
    ...
    constexpr int PROGRESS_INTERVAL = 5;
    ...
    for (int i = 0; i < max_tok; i++) {
        ...
        if (llama_vocab_is_eog(vocab, new_token)) {
            ...
            if (!pending_text.empty()) {
                report_progress(env, thiz, progressMid, "generating", i, pending_text.c_str());
            }
            break;
        }
        ...
        if ((i + 1) % PROGRESS_INTERVAL == 0) {
            ...
            report_progress(env, thiz, progressMid, phase_buf, i + 1, pending_text.c_str());
            pending_text.clear();
        }
...
extern "C"
JNIEXPORT jstring JNICALL
Java_dev_nutting_pocketllm_llm_LlmEngine_nativeInferText(
    JNIEnv *env, jobject thiz,
    ...
    if (caught_signal != 0) {
        g_in_guarded_section = false;
        const char *sig_name = (caught_signal == SIGSEGV) ? "SIGSEGV" : "SIGBUS";
        LOGe("Caught %s during nativeInferText — poisoning native state", sig_name);
        poison_native_state();
        jclass exc = env->FindClass("java/lang/RuntimeException");
        char msg[128];
        snprintf(msg, sizeof(msg),
    ...
    // Build prompt with chat template
    const auto *prompt_str = env->GetStringUTFChars(jPrompt, nullptr);
    std::string user_content = prompt_str;
    env->ReleaseStringUTFChars(jPrompt, prompt_str);
    ...
    // Tokenize text-only prompt (no image)
    const auto *vocab = llama_model_get_vocab(g_model);
    std::vector<llama_token> tokens = common_tokenize(g_context, full_prompt, true, true);
    LOGi("Text-only prompt tokenized: %zu tokens", tokens.size());
    ...
    LOGi("Text inference generated %zu chars", result.size());
    return env->NewStringUTF(result.c_str());
}
...
Java_dev_nutting_pocketllm_llm_LlmEngine_nativeUnload(JNIEnv *, jobject) {
    // If poisoned, the pointers are already null — just clear the flag.
